/**
 * MCP Server function for List Sources
 */
...
class Get_SourcesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_SourcesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("sort_by")) {
            queryParams.add("sort_by=" + args.get("sort_by"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_sources" + queryString;
    ...
    public static MCPServer.Tool createGet_SourcesTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getGet_SourcesHandler(config));
    }

/**
 * MCP Server function for List Tasks
 */
...
class Get_TasksMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_TasksHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("sort_by")) {
            queryParams.add("sort_by=" + args.get("sort_by"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_tasks" + queryString;
    ...
    public static MCPServer.Tool createGet_TasksTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getGet_TasksHandler(config));
    }
